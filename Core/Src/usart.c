...
#include "crsf.h"
#include "mtf01.h"
extern uint8_t uartRx[16];
extern uint8_t u10_rx_data[1];
extern char str[100];
uint8_t uartTemp[64] = {0};
int  uartUpdate = 0;
int Heartbeats=0;
crsf_channels_t rcData;
crsfLinkStatistics_t lqData;
MICOLINK_MSG_t  msg;
MICOLINK_PAYLOAD_RANGE_SENSOR_t mtf_01;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
    Heartbeats=1;
		uartUpdate = 1;//update flag set 1
		for (uint8_t i = 0; i < 16; i ++)
		{//move new 16 bytes to old bytes
			uartTemp[i] = uartTemp[i + 16];
			uartTemp[i + 16] = uartTemp[i + 32];
			uartTemp[i + 32] = uartTemp[i + 48];
			uartTemp[i + 48] = uartRx[i];
		...
		HAL_UART_Receive_IT(&huart7, uartRx, 16);//restart inturrupt
		}else
    {
      Heartbeats=0;
    }
		
		if(huart->Instance == huart10.Instance) 
    {micolink_decode(&mtf_01,&msg,u10_rx_data[0]);
//	__HAL_UART_ENABLE_IT(&huart1, UART_IT_ERR);
//__HAL_UART_ENABLE_IT(&huart1, UART_IT_RXNE);
			HAL_UART_Receive_IT(&huart10, u10_rx_data,1);
//			if(HAL_UART_Receive_IT(&huart10, u10_rx_data,1)!=HAL_OK)
//{
// __HAL_UART_ENABLE_IT(&huart10, UART_IT_ERR);
//}
		}
...
{
	if(huart==&huart10) HAL_UART_Receive_IT(&huart10, u10_rx_data,1);
		if(huart==&huart7) HAL_UART_Receive_IT(&huart7, uartRx, 16);
}
